...
(function () {
  ...
  (function menus() {
    ...
    triggers.forEach(function (trigger) {
      ...
      var hoverTimer;
      // Hover already opened the panel by the time the pointer reaches the
      // button, so a click used to close it and a second click reopened it.
      // A click on a hover-opened panel now leaves it open; hover-leave closes.
      ...
      item.addEventListener('mouseleave', function () {
        if (!DESKTOP.matches) return;
        hoverTimer = window.setTimeout(function () { close(trigger); openedByHover = false; }, 180);
      });
  ...
  (function quoteForm() {
    ...
    var MESSAGES = {
      ...
      'q-message': 'Tell us how we can help'
    };
  ...
  (function contactForm() {
    ...
    var MESSAGES = {
      ...
      'c-phone': 'Enter your phone number',
      'c-message': 'Tell us how we can help'
    };
  ...
  /* FAQ accordion — one answer open at a time. -------------------------------
     The markup is native <details>, so without this every card opens and stays
     open. Listening on `toggle` rather than on the summary's click keeps
     keyboard opening working, and the guard on `open` means closing a card
     never touches the others. */
  (function faqAccordion() {
    var groups = document.querySelectorAll('.faq__grid, .qa-list');
    if (!groups.length) return;

    groups.forEach(function (group) {
      var items = Array.prototype.slice.call(group.querySelectorAll('details.qa'));
...
/* Footer accordions on phones: tapping a column heading opens its list.
   Desktop keeps the four open columns. */
(function footerAccordion() {
  var cols = document.querySelectorAll('.site-footer__col');
  if (!cols.length) return;
  var mq = window.matchMedia('(max-width: 40em)');
  cols.forEach(function (col) {
    var head = col.querySelector('.site-footer__heading');
    if (!head) return;
